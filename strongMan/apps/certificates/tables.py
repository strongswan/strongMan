...
class UserCertificateTable(tables.Table):
    ...
    subject = tables.Column(verbose_name="DistinguishedName",
                            attrs={'th': {'class': "hidden-sm hidden-xs"}, 'td': {'class': "hidden-sm hidden-xs"}})
    ...
    def render_name(self, record):
        return render_to_string('certificates/widgets/name_column.html', {'record': record}, request=self.request)
    ...
    def render_removebtn(self, record):
        ...
        return render_to_string('certificates/widgets/remove_column.html', {'id': record.id, "is_vici": is_vici},
